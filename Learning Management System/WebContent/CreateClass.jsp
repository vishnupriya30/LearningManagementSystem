...
<table>
    ...
    <a href="HomePage.jsp" style="text-decoration:none"><h1 style="padding-top:20px;padding-left:30px;font-family: 'Alegreya', serif; font-size:50px;color:white"> Vishnu Classroom </h1></a>
     ...
     <th style="padding-left:900px;padding-top:25px;">
       <button style="background-color:white;border-radius:10px;border-color:#3a7bd5;height:50px;width:150px;font-size:20px" id = "myBtn" type="submit">Create Class</button>
  ...
  <form action="CreateClassServlet" Method="post">
     ...
     <input type="text" id="subjectName" name="subjectName" placeholder="Subject Name" required>
     <input type="submit" value="Create Class" style="margin-top:20px">
  </form>
</div>
   </div>
     </th>
    ...
    List<Class> classes = (List<Class>)ClassDao.getAllClasses(userId);
    int tclass = classes.size();
    if(tclass == 0){
%>
...
<div class="column" style="margin-top:30px">
    <div class="card">
      ...
      <div class="container">
        ...
        <p class="title">Faculty Name: <%=c.getFacultyName()%></p>
        <p><%=c.getClassName()%></p>
        <p>Class Code:<%=c.getClassCode()%></p>
        <form action="ClassServlet?classId=<%=c.getClassId()%>" Method="post">
        <p><input type="submit" class="button" value="Enter Class Room"></p>
        </form>
      </div>
    </div>
  </div>
