...
<div style="width:70%;background-color:#3a7bd5;height:60px;border-style:solid;border-radius:3px;border-color:white;margin-top:-620px;margin-left:220px">
  <center style="margin-top:10px;font-size:30px;color:white;font-family: 'Alegreya', serif; color:white;text-shadow:2px 2px #3a7bd5;">NOT &nbsp&nbspATTEMPTED &nbspSTUDENTS </center>
   <% 
      ...
      List<Integer> attemptedUserId = new ArrayList<Integer>();
      for(int i = 0;i < correction.size(); i++){
    	  attemptedUserId.add((correction.get(i)).getUserId());
      }
      ...
      List<User> notAttemptedUsers = new ArrayList<User>();
      for(int i = 0;i < allUsers.size(); i++){
    	  if(!attemptedUserId.contains(allUsers.get(i)))
    		  notAttemptedUsers.add(UserDao.getUser(allUsers.get(i)));
      }
      Collections.sort(notAttemptedUsers);
     %>
</div>
...
<%for(int i = 0;i < notAttemptedUsers.size(); i++){%>
<tr>
<td><%=notAttemptedUsers.get(i).getRegisterNo()%></td>
<td><%=notAttemptedUsers.get(i).getFirstName()+" "+notAttemptedUsers.get(i).getLastName()%></td>
</tr>
<%} %>
</table>
</div>
